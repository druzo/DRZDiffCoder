...
<p align="center">
  <a href="releases/0.1.1/"><img alt="release" src="https://img.shields.io/badge/release-v0.1.1-blueviolet?style=for-the-badge"></a>
...
> **Download** the pre-built artifacts from [`releases/0.1.1/`](releases/0.1.1/) and pick your platform. Every artifact ships with a per-folder `SHA256SUMS` — verify with `sha256sum -c SHA256SUMS`.
...
1. Download `drzdiff_0.1.1_x64.msi` from `releases/0.1.1/windows-x86_64/`.
...
```bash
cd linux-x86_64
sudo dpkg -i drzdiff_0.1.1_amd64.deb
...
```bash
cd linux-x86_64
chmod +x drzdiff-0.1.1-x86_64.AppImage
./drzdiff-0.1.1-x86_64.AppImage
```
...
```bash
cd linux-arm64
sudo dpkg -i drzdiff_0.1.1_arm64.deb
...
Reproduce the `releases/0.1.1/` folder from this checkout:
...
```bash
VERSION=0.1.1 PLATFORMS="windows linux-x86_64 linux-arm64 darwin-x86_64 darwin-arm64" \
  ./scripts/release.sh
```

The pipeline is **idempotent** — re-running with the same `VERSION` overwrites prior artifacts.

CI wires this up via `.github/workflows/release.yml`: push a tag like `v0.1.1` and the workflow builds every platform, attaches all installers to a GitHub Release, and uploads `SHA256SUMS`.
...
Binaries are unsigned for v0.1.1. Click *More info → Run anyway*. Production deployments should sign with an Authenticode certificate.
...
- [Release notes for v0.1.1](releases/0.1.1/RELEASE-NOTES.md).
