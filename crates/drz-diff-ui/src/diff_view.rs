...
mod tests {
    ...
    // The top menu bar fires `EditAction` variants into `apply_edit`. These
    // tests prove the dispatch lands on the right ViewModel command — the
    // exact wiring the user reported as broken ("editor menu is not working").

    use drz_editor::CodeEditor;
    use drz_viewmodel::{EditorViewModel, LanguageId};

    #[test]
    fn apply_edit_undo_routes_to_viewmodel_and_restores_text() {
        let ctx = egui::Context::default();
        let mut vm = EditorViewModel::from_text("alpha\n", LanguageId::PlainText);
        vm.edit(5, 5, "X");
        assert_eq!(vm.document_text(), "alphaX\n");
        assert!(vm.can_undo());

        let mut editor = CodeEditor::new();
        apply_edit(&mut editor, &mut vm, EditAction::Undo, &ctx);
    ...
    #[test]
    fn apply_edit_redo_replays_the_undone_edit() {
        let ctx = egui::Context::default();
        let mut vm = EditorViewModel::from_text("alpha\n", LanguageId::PlainText);
        vm.edit(5, 5, "X");
        let mut editor = CodeEditor::new();
        ...
        apply_edit(&mut editor, &mut vm, EditAction::Redo, &ctx);
        assert_eq!(vm.document_text(), "alphaX\n");
        assert!(vm.can_undo());
        assert!(!vm.can_redo());
    }
    ...
    #[test]
    fn apply_edit_select_all_sets_selection_to_full_document() {
        let ctx = egui::Context::default();
        let mut vm = EditorViewModel::from_text("abc\ndef\n", LanguageId::PlainText);
        let mut editor = CodeEditor::new();

        apply_edit(&mut editor, &mut vm, EditAction::SelectAll, &ctx);

        let sel = editor.selection().expect("SelectAll must set selection");
        assert!(sel.is_selected());
        assert_eq!(sel.ordered().0, (0, 0));
        let last_line = vm.len_lines().saturating_sub(1);
        let last_len = vm.line(last_line).len();
        assert_eq!(sel.ordered().1, (last_line, last_len));
    }
}
